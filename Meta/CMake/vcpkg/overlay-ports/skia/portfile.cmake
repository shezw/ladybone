...
vcpkg_from_github(
    ...
    PATCHES
        ...
        gcc8-cxx20-flag.patch
        arm-skcms-portable.patch
        arm-rasterpipeline-scalar.patch
)
...
function(install_skia_static_archive_from_objects archive_name build_dir destination_dir)
    set(object_paths)
    foreach(object_path IN LISTS ARGN)
        if(EXISTS "${object_path}")
            list(APPEND object_paths "${object_path}")
        else()
            message(FATAL_ERROR "Expected Skia object '${object_path}' for ${archive_name} does not exist")
        endif()
    endforeach()
    ...
    vcpkg_execute_required_process(
        COMMAND "${VCPKG_DETECTED_CMAKE_AR}" qc "${archive_path}" ${object_paths}
        WORKING_DIRECTORY "${build_dir}"
        LOGNAME "archive-${archive_name}"
    )
    vcpkg_execute_required_process(
        COMMAND "${VCPKG_DETECTED_CMAKE_RANLIB}" "${archive_path}"
        WORKING_DIRECTORY "${build_dir}"
        LOGNAME "ranlib-${archive_name}"
    )
endfunction()
